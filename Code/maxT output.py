...
def Calc_tmax_mean(file_dir, file_year): 
    
    """
    Read a year's file, calculate the mean of a March, April, May, June tmax,
    also Nov, Dec, Jan, and Feb tmax
    ...
    jan_tmax = ds.sel(time=slice(jan_start, jan_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    feb_tmax = ds.sel(time=slice(feb_start, feb_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    mar_tmax = ds.sel(time=slice(mar_start, mar_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    apr_tmax = ds.sel(time=slice(apr_start, apr_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    may_tmax = ds.sel(time=slice(may_start, may_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    jun_tmax = ds.sel(time=slice(jun_start, jun_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    jul_tmax = ds.sel(time=slice(jul_start, jul_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    nov_tmax = ds.sel(time=slice(nov_start, nov_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
    dec_tmax = ds.sel(time=slice(dec_start, dec_end))['tmax'].mean(dim=('lat', 'lon', 'time'))
...
tmp_store = Calc_tmax_mean(file_dir = file_dir, file_year = 1990)
tmp_store2 = Calc_tmax_mean(file_dir = file_dir, file_year = 1991)
...
for i in range(len(test_year)):
    ...
    current_dict = Calc_tmax_mean(file_dir = file_dir, file_year = test_year[i])
...
for i in range(len(years_uWant)):
    # Call the function to generate a dictionary
    current_dict = Calc_tmax_mean(file_dir = file_dir, file_year = years_uWant[i])
...
combined_dataframe.to_csv("monthly tmax.csv")